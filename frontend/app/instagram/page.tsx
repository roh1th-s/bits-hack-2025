...
export default function UrlSearchPage({ className }: UrlSearchProps): JSX.Element {
  ...
  const [fake, setFake] = useState<string | null>(null);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>): void => {
    ...
    try {
      ...
      setFake(Math.random() > 0.5 ? "fake" : "not fake");
    } catch (error) {
      setError("Please provide valid link");
    }
  };
  ...
  return (
    <div className={`min-h-screen bg-background p-4 ${className || ''}`}>
      ...
      <div className="max-w-2xl mx-auto mt-20">
        ...
        {/* Text showing fake or not fake */}
        <p className="mt-8 text-center text-5xl text-muted-foreground">
          {fake == "fake" ? "That shit's fake" : fake == "not fake" ? "That shit's real" : ""}
        </p>
      </div>
    </div>
  );
